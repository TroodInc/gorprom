...
const ProductEdit = ({ host }) => {
  ...
  const formStoreName = 'product' + id
  let formStore = store.formStore.has(formStoreName) ? store.formStore.get(formStoreName) : undefined

  if (!formStore) {
    const { get, loaded } = store.callHttpQuery(custodianApiPath + 'product/' + id)
    if (loaded) {
      ...
      const productEditData = {
        ...
        photo_set: productData.photo_set,
        teaser: productData.teaser,
    ...
    <>
      ...
      <div className={styles.controls}>
        <Button
          ...
          onClick={() => {
            form.submit(custodianApiPath + 'product/' + id, 'PATCH').then(({ data }) => {
